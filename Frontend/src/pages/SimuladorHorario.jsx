...
export default function SimuladorHorario({ archivosListos = false }) {
  ...
  const iniciarPolling = useCallback((id) => {
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        const { estado: estadoJob, mensaje } = data;
  ...
  const iniciarSimulacion = () => {
    ...
    if (!storedJobId) {
      agregarLog("No se encontró un jobId en la sesión.", "error");
  ...
  const descargarHorario = async () => {
    ...
    try {
      ...
      const blob = await obtenerResultado(id);
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                disabled={
                  corriendo || (!archivosListos && estado === ESTADO.IDLE)
                }
                className={`
                  text-sm
                  font-black
